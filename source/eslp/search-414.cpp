...
{
	return (hiFactor > 64000) ? 16000 :
		16000 + (64000 - hiFactor) / 4;
}
...
void Search414::InitFirstTask(WorkerTask& task, const std::vector<unsigned>& startFactors)
{
	...
	m_ProgressMask = (f < 32000) ? 511 : (f < 54000) ? 127 :
		(f < 80000) ? 63 : 15;
}
...
AML_NOINLINE void Search414::SearchFactors(Worker* worker, const NumberT* powers)
{
	...
	// части. Но так все эти 3 коэф-та кратны 5, то мы будем перебирать значения, уменьшенные в 5 раз
