...
AML_NOINLINE void Search413::SearchFactors(Worker* worker, const NumberT* powers)
{
	...
	for (unsigned k1 = f1, dt = 0; k1 < k[0]; k1 += delta[dt], dt ^= 1)
	{
		...
		// Если коэффициент в левой части кратен 3, то ни один из коэффициентов правой части не может быть
		// кратен 3. Если k[1] не кратен 5, то оставшиеся 2 коэффициента правой части должны быть кратны 5.
		// В этом случае значение zd должно быть кратно 5^4. Если это не так, то решений не будет
		if ((zm3 && !(k1 % 3)) || (k1 % 5 && zd % 625))
			continue;
