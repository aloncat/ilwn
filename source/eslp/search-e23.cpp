...
bool SearchE23::MightHaveSolution(const Task& task) const
{
	...
	// Оптимизация одновременной кратности 3 и 5 для 4-й степени не имеет смысла, т.к. даёт слабый
	// эффект и только для случая rightCount == 3. Оптимизации для 6-й степени не имеют смысла
