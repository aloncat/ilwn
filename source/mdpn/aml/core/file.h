...
public:
	...
	// Читает не более bytesToRead байт из файла в buffer. Если во внутреннем буфере
	...
	virtual bool Read(void* buffer, size_t bytesToRead, size_t& bytesRead);
	// Записывает bytesToWrite байт из buffer в файл. По возможности (при достаточности
...
protected:
	...
	bool ReadFromFile(void* buffer, size_t bytesToRead, size_t& bytesRead);
	bool WriteToFile(const void* data, size_t bytesToWrite);

protected:
	uint8_t* m_Buffer;				// Указатель на внутренний буфер
