...
public:
	...
	// достиг 1/5 максимального размера набора, то будет удалён первый блок этой части. Новое число всегда
	// добавляется после удаления элементов (если они удалялись)
...
protected:
	...
	// Количество блоков в расчёте на одну часть, при котором происходит удаление элементов. Значение
	// должно лежать в пределах от 2 до (EIGHTH_CHUNK_C - 2). Чем оно будет выше, тем выше будет среднее
	// количество элементов на один хеш. Более высокие значения позволяют задействовать больше памяти и
	// увеличить ёмкость набора, однако значения выше 10 замедляют работу функции Exists
	static constexpr size_t CLEAR_GAIN = 10;

	// При текущих значениях EIGHTH_CHUNK_C (16), CLEAR_GAIN (10) и CHUNK_SIZE (X-5), на каждый хеш приходится в
	// среднем по 3.5 элемента. Это обеспечивает наилучший баланс между скоростью работы функции Exists и размером
	// хеш-таблицы (26.7% от максимального кол-ва расходуемой памяти). Значение (X-6) немного увеличивает скорость
	// работы (от 1 до 3 элементов на хеш, в среднем 2.25 элемента), значение (X-4) приводит к значительному
	// снижению скорости работы (от 1 до 9 элементов на хеш, в среднем 6 элементов на хеш)
	...
	struct Item {
		...
		uint32_t next;	// Индекс следующего числа в m_Chunk (LAST если этот элемент последний)
