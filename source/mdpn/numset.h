...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//   NumberSet - хеш-таблица чисел FixNumber
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
protected:
	...
	// лежать в пределах от 2 до (EIGHTH_CHUNK_C - 2). Чем оно будет выше, тем выше будет среднее количество
	...
	// При значениях EIGHTH_CHUNK_C (16), CLEAR_GAIN (10) и CHUNK_SIZE (X-5), на каждый хеш приходится
	// в среднем по 3.5 элемента. Это обеспечивает наилучший баланс между скоростью работы функции Exists
	// и размером хеш-таблицы (26.7% от максимального объёма расходуемой памяти). Значение (X-6) немного
	// увеличивает скорость работы (в среднем 2.25 элемента на хеш), значение (X-4) приводит к
	...
	struct Item {
		...
		uint32_t next = LAST;	// Индекс следующего числа в m_Chunk (LAST если этот элемент последний)
	...
	size_t m_RCount = 0;		// Суммарное кол-во перенесённых в m_TableA элементов
	size_t m_RCountA[8];		// Кол-во эл-в, перенесённых в m_TableA из блоков для каждой из частей
	unsigned m_HBits = 0;		// Кол-во используемых бит хеша в данный момент
