...
{
	constexpr size_t KNOWN_DIGIT_C = 22;
	...
	// имеющихся данных 21 проверенного диапазона), чтобы спрогнозировать достаточную
	// глубину поиска для диапазонов, проверка которых ещё не завершена
	const float depth = 13.881f * digitC - 14.543f + 6.f * 11.49f;
...
unsigned StepHelper::GetHighest(const Number& num)
{
	...
	static const unsigned stepA[KNOWN_DIGIT_C + 1] = { 0, 0, 2, 24, 24, 24, 55, 64,
		96, 96, 98, 109, 149, 149, 188, 188, 201, 201, 236, 236, 261, 261, 261 };
...
void StepHelper::InitFoundRanges()
{
	...
	R(20) << 223 << R(237, 257);
	R(21) << R(237, 252);
	// TODO: дополнить набор при необходимости (после окончания проверки
	// диапазона 22-значных чисел в случае обнаружения новых шагов >=262)
	R(22) << R(237, 250);
	// TODO: скорректировать значения при необходимости (после окончания проверки диапазона 22-значных
	// чисел в случае обнаружения в нём шагов из этого набора; а также после окончания проверки
	// 23-значных чисел в случае обнаружения среди них новых шагов >= 262)
	R(23) << 237 << R(241, 249);

	// Ненайденные шаги (обновятся при обнаружении нового
	// шага в диапазоне 22-, 23-значных чисел или старше)
	R(~0u) << 237 << R(241, 245) << 248 << R(262, Const::MAX_STEP);
}
